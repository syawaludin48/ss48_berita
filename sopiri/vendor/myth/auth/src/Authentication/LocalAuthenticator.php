...
class LocalAuthenticator extends AuthenticationBase implements AuthenticatorInterface
{
    ...
    public function attempt_mahasiswa(array $credentials, bool $remember = null): bool
    {
        ...
        if (empty($this->user))
        {
            ...
            $this->recordLoginAttempt($credentials['email'] ?? $credentials['nim'], $ipAddress, $this->user->id ?? null, false);
        ...
        if (! $this->user->isActivated())
        {
            ...
            $param = http_build_query([
                'login' => urlencode($credentials['email'] ?? $credentials['nim'])
            ]);
    ...
    public function attempt(array $credentials, bool $remember = null): bool
    {
        ...
        if (! $this->user->isActivated())
        {
            // Always record a login attempt, whether success or not.
            $ipAddress = Services::request()->getIPAddress();
            $this->recordLoginAttempt($credentials['email'] ?? $credentials['username'] ?? $credentials['nim'], $ipAddress, $this->user->id ?? null, false);

            $param = http_build_query([
                'login' => urlencode($credentials['email'] ?? $credentials['username'] ?? $credentials['nim'])
            ]);
