...
use Modules\ManageMenuWebsite\Models\ModelMenu as ModelMenu;
...
class DataSubSubMenu extends BaseController
{
	...
	public function sub_sub_menu()
	{
		...
		$sub_sub_menu = $this->ModelMenu->get_sub_sub_menu();
		$sub_menu = $this->ModelMenu->get_sub_menu();

		$data = [
			'seg' => $this->request->uri->getSegments(),
			'pretitle' => 'Tabel Data',
			'title' => 'Data Sub Sub Menu Webiste',
			'sub_sub_menu' => $sub_sub_menu,
			'sub_menu' => $sub_menu,
		];
		
		return view('Modules\ManageMenuWebsite\Views\sub_sub_menu',$data);
	}

	public function sub_sub_menu_save()
	{
		...
		$rules = [
			...
			'name' => [
				'rules'  => 'required|is_unique[sub_sub_menu_website.nama_sub_sub_menu]',
				'errors' => [
		...
		$this->ModelMenu->save_sub_sub_menu($id_menu,$nama_menu,$slug_menu,$link,$status,$no_urut);
		
		$this->session->setFlashdata('sukses', 'Melakukan Tambah Data Sub Menu !' );
		return redirect()->to('/manage-sub-sub-menu-website'); 

	}

	public function sub_sub_menu_update()
	{
		...
		$rules = [
			...
			'name' => [
				'rules'  => 'required|is_unique[sub_sub_menu_website.nama_sub_sub_menu,random,{random_sub}]',
				'errors' => [
					'required'  => '<b>{field}</b> harus di isi !!!',
					'is_unique' => '<b>{field}</b> <b>{value}</b> sudah terdaftar !!!'
				]
		...
		$this->ModelMenu->edit_sub_sub_menu($id_menu,$id_rand,$nama_menu,$slug_menu,$link,$status,$no_urut);
	...
	public function sub_sub_menu_delete()
	{
		 
        $id_rand	= filterdata($this->request->getPost('random_sub'));

		$this->ModelMenu->delete_sub_sub_menu($id_rand);
