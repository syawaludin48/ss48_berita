...
class DataSubMenu extends BaseController
{
	...
	public function sub_menu()
	{
		...
		return view('Modules\ManageMenu\Views\sub_menu',$data);
	}
	...
	public function sub_menu_save()
	{
		...
		$this->ModelMenu->save_sub_menu($id_menu,$nama_menu,$slug_menu,$link,$manage,$status,$no_urut);
	...
	public function sub_menu_update()
	{
		...
		$rules = [
			...
			'name' => [
				'rules'  => 'required|is_unique[sub_menu.nama_sub_menu,random,{random_sub}]',
				'errors' => [
					'required'  => '<b>{field}</b> harus di isi !!!',
					'is_unique' => '<b>{field}</b> <b>{value}</b> sudah terdaftar !!!'
				]
		...
		$slug_menu    	= url_title(strtolower($nama_menu));
        $link		= filterdata($this->request->getPost('link'));
        $value		= filterdata($this->request->getPost('value'));
        $manage		= filterdata($this->request->getPost('manage'));
        $status		= filterdata($this->request->getPost('status'));
        $no_urut		= filterdata($this->request->getPost('nomor'));
		
		$this->ModelMenu->edit_sub_menu($id_menu,$id_rand,$nama_menu,$slug_menu,$link,$manage,$status,$no_urut);
